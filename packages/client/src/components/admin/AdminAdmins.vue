      ...
      <table>
        ...
        <tbody>
          <tr v-for="adm in admins" :key="adm.username">
            <td>{{ adm.username }}</td>
            <td><input type="password" v-model="passwords[adm.username]" placeholder="New password" /></td>
            <td class="actions">
              <button class="btn btn-primary sm" @click="updatePassword(adm.username)">Save</button>
              <button v-if="adm.username !== 'admin'" class="btn btn-danger sm" @click="deleteAdmin(adm.username)">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
...
async function createAdmin() {
  ...
  try {
    ...
    await load()
    toast('Admin created.')
  } catch (e) { addError.value = e instanceof ApiError ? e.message : 'Error.' }
}
</script>

<style lang="scss" scoped>
@use '../../styles/variables' as *;
.panel { padding: 1.25rem; display: flex; flex-direction: column; gap: 1.5rem; }
table { width: 100%; border-collapse: collapse; font-size: .85rem; max-width: 600px;
...
.add-section { max-width: 500px; h3 { font-size: .875rem; font-weight: 600; margin-bottom: .75rem; } }
.add-row { display: flex; gap: .5rem; input { flex: 1; } }
.error { font-size: .82rem; color: $danger; margin-top: .5rem; }
