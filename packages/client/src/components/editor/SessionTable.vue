        ...
        <tr v-for="row in rows" :key="row.clock_in" :class="{ incomplete: row.incomplete, pending: isPending(row) }">
          ...
          <td class="time-cell" @click="startEdit(row, 'clock_in')">
            <template v-if="editing?.key === row.clock_in + ':in'">
              <input
                ref="activeEditInput"
                type="datetime-local"
                :value="toLocalInput(row.clock_in)"
            ...
            <template v-else>
              ...
              <span v-if="isPendingField(row, 'clock_in')" class="pending-val">
                {{ pendingNew(row, 'clock_in') }}
              </span>
            </template>
          </td>
          <td class="time-cell" @click="!row.incomplete ? startEdit(row, 'clock_out') : null">
            <template v-if="editing?.key === row.clock_in + ':out'">
              <input
                ref="activeEditInput"
                type="datetime-local"
                :value="row.clock_out ? toLocalInput(row.clock_out) : ''"
            ...
            <template v-else>
              ...
              <span v-if="isPendingField(row, 'clock_out')" class="pending-val">
                {{ pendingNew(row, 'clock_out') }}
              </span>
            </template>
          </td>
          <td>{{ row.duration ?? '—' }}</td>
          <td v-if="isAdmin">
            <button
              ...
              class="del-btn"
              title="Delete"
              @click.stop="$emit('delete-row', row.clock_in)"
            >✕</button>
          </td>
        </tr>
        <tr v-if="!rows.length">
...
import { toLocalInput, toTimestamp } from '../../utils/time.js'
...
const activeEditInput = ref<HTMLInputElement | null>(null)
