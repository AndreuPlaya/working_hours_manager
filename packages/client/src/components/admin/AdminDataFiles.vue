    ...
    <table v-if="files.length">
      ...
      <tbody>
        <tr v-for="f in sortedFiles" :key="f.name">
          ...
          <td><button class="btn btn-danger sm" @click="deleteFile(f.name)">Delete</button></td>
        </tr>
      </tbody>
    </table>
...
async function load() { files.value = await api.admin.rawFiles() }
...
async function deleteFile(name: string) {
  ...
  try { await api.admin.deleteFile(name); await load(); toast('Deleted.') }
...
.drop-zone {
  border: 2px dashed $input-border; border-radius: .5rem; padding: 2rem;
  text-align: center; cursor: pointer; color: $text-muted; font-size: .875rem;
  transition: border-color .15s, background .15s;
  &:hover, &.dragging { border-color: $accent; background: $selected-bg; color: $accent; }
}
...
table { width: 100%; border-collapse: collapse; font-size: .85rem;
  ...
  th.sortable { cursor: pointer; &:hover { color: $text; } }
}
